...
public class Layer {
	...
	Layer(Dimension d, Point p, int zoom, int layerId, DBValues DB) {
		...
		realDimension.setSize(round(d.width * zoomFactor), round(d.height
				* zoomFactor));
				...
				if (temp.hasImage()) {
					if (zoomFactor != 1) {
						...
						temp.setImage(temp.getImage().getScaledInstance(
								round(resizedX), round(resizedY),
								Image.SCALE_DEFAULT));
						temp.setSize(new Dimension(round(resizedX),
								round(resizedY)));
					...
					if (xPaint < 0 || yPaint < 0) {
						...
						BufferedImage cutTile = new BufferedImage(width,
								height, BufferedImage.TYPE_INT_ARGB);
						if (xToCut > 0 && yToCut > 0) {
							cutTile.getGraphics().drawImage(temp.getImage(), 0,
									0, null);
						}
						if (xToCut > 0 && yToCut == 0) {
							cutTile.getGraphics().drawImage(temp.getImage(), 0,
						...
						if (xToCut == 0 && yToCut > 0) {
							cutTile.getGraphics().drawImage(temp.getImage(),
									xToCut, 0, null);
						}
						int newWidth = width - xToCut;
						int newHeight = height - yToCut;
						temp.setImage(cutTile.getSubimage(round(xToCut),
								round(yToCut), newWidth, newHeight));
						temp.setXFrom(temp.getXFrom() - xToCut);
						temp.setYFrom(temp.getYFrom() - yToCut);
						map.getGraphics().drawImage(temp.getImage(),
								xPaint + xToCut, yPaint + yToCut, null);
					} else {
						map.getGraphics().drawImage(temp.getImage(), xPaint,
								yPaint, null);
					}
				}
			}
	...
	public void setColor(Color color, Color coloralpha) {
		...
		ByteLookupTable blut = new ByteLookupTable(0, new byte[][] { cR, cG,
				cB, cA });
